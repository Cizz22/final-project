import json
import unittest

from models.abc import db
from repositories import AnnouncementRepository, UserRepository
from server import server

class TestAnnouncement(unittest.TestCase):

    user_data = {
            "name": "test",
            "email": "test@email.com",
            "phone_number": "081234567890",
            "password": "password",
            "type": "seller"
        }

    @classmethod
    def setUpClass(cls):
        server.config['SERVER_NAME'] = 'localhost:5053'
        cls.client = server.test_client()

    def setUp(self):
        self.app_context = server.app_context()
        self.app_context.push()
        db.create_all()

    def tearDown(self):
        db.session.remove()
        db.drop_all()
        self.app_context.pop()

    def get_token(self):
        UserRepository.create(self.user_data["name"], self.user_data["email"], self.user_data["phone_number"], self.user_data["password"], self.user_data["type"])

        login_admin = self.client.post(
            "/sign-in",
            data = json.dumps({
                "email": self.user_data["email"],
                "password": self.user_data["password"]
            }),
            content_type = "application/json"
        )

        token = json.loads(login_admin.data.decode("utf-8"))["token"]

        return token

    def test_get(self):

        AnnouncementRepository.create("announcement A", "announcement A")
        AnnouncementRepository.create("announcement B", "announcement B")

        announcements = AnnouncementRepository.get_all()

        data = [
            {
                "id": str(announcement.id),
                "title": announcement.title,
                "content": announcement.title
            } for announcement in announcements
        ]

        response = self.client.get(
            "/announcements"
        )

        response_json = json.loads(response.data.decode("utf-8"))

        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response_json,
            {
                "data": data
            }
        )

    def test_post(self):

        token = self.get_token()

        response = self.client.post(
            "/announcements",
            headers = {
                "Authentication": token
            },
            data = json.dumps({
                "title": "announcement A",
                "content": "announcement A"
            }),
            content_type = "application/json"
        )

        response_json = json.loads(response.data.decode("utf-8"))

        announcement = AnnouncementRepository.get_by(title="announcement A").one_or_none()

        self.assertEqual(response.status_code, 201)
        self.assertEqual(
            response_json,
            {
                "message": "Announcement added"
            }
        )
        self.assertEqual(announcement.title, "announcement A")

    def test_put(self):

        AnnouncementRepository.create("announcement A", "announcement A")
        announcement = AnnouncementRepository.get_by(title="announcement A").one_or_none()
        announcement_id = str(announcement.id)

        token = self.get_token()

        response = self.client.put(
            f"/announcements",
            headers = {
                "Authentication": token
            },
            data = json.dumps({
                "id": announcement_id,
                "title": "announcement B",
                "content": "announcement B"
            }),
            content_type = "application/json"
        )

        response_json = json.loads(response.data.decode("utf-8"))

        new_announcement = AnnouncementRepository.get_by(id=announcement_id).one_or_none()

        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response_json,
            {
                "message": "Announcement updated"
            }
        )
        self.assertEqual(new_announcement.title, "announcement B")
        self.assertEqual(new_announcement.content, "announcement B")

    def test_delete(self):

        AnnouncementRepository.create("announcement A", "announcement A")
        announcement = AnnouncementRepository.get_by(title="announcement A").one_or_none()
        announcement_id = str(announcement.id)

        token = self.get_token()

        response = self.client.delete(
            f"/announcements/{announcement_id}",
            headers = {
                "Authentication": token
            }
        )

        response_json = json.loads(response.data.decode("utf-8"))

        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response_json,
            {
                "message": "Announcement deleted"
            }
        )
        #self.assertNotEqual(announcement.deleted_at, None)